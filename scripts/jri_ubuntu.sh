...
function install_jri_module(){
  ...
  mv jri-publisher-master jri_publisher
	...
	/usr/share/webmin/install-module.pl /tmp/jri_publisher.wbm.gz
	rm -f /tmp/jri_publisher.wbm.gz

  mv /usr/share/webmin/jri_publisher/gen_jri_report.sh /usr/local/bin/
...
function install_jri_module_deps(){
	apt-get install -y haveged mutt zip postfix
